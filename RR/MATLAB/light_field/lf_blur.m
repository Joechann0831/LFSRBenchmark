function Y = lf_blur(X,gaussWindow,GaussSig)


% Initialize the gaussian filder with window-size and standard-deviation
% defined in the param structure
psf = fspecial('gaussian', gaussWindow,GaussSig);
...
for n = 1:size(X,4)
    for m = 1:size(X,5)
        % Get the high-resolution sub-aparture image
        I = double(X(:,:,:,n,m));
        
        % Blur the image using the NCSR code
        I_blur = blur('fwd', I, psf);
        
                % Put the low-quality lightfield into the low-res lightfield
        Y(:,:,:,n,m) = uint8(I_blur);
    end
end
Y = uint8(Y);
...
%                     removed.
% This function was extracted from the NCSR noise model and we set the
% noise variance to zero so that only blurring is considered.
